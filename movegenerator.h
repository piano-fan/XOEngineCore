...
namespace XO{
    class MoveGenerator{
        ...
        protected:
            ...
            BaseHelper(const SquareObserver& obs, Piece own, TProperty sq_class)
                    :m_obs(&obs), m_own(own), m_square_class(sq_class)
        ...
        class UnlimitedHelper : public BaseHelper{
            ...
            iterator m_end;
        public:
            UnlimitedHelper(const SquareObserver& obs, Piece own, TProperty sq_class)
                    :BaseHelper(obs, own, sq_class)
            {
                Reset();
            }

            void Reset() override{
                auto& sqlist = m_obs->GetTrackedSquares(m_own, m_square_class);
        ...
        public:
            D4BlockerHelper(const SquareObserver& obs, Piece own, TProperty sq_class)
                    :BaseHelper(obs, own, sq_class)
            {
                m_center = m_obs->AnySquare(OppositePiece(m_own), TProperty::D4);
                Reset();
                m_empty = !m_valid;
            }
            ...
            void Next() override{
                ...
                for(; m_offset.Valid(); ++m_offset){
                    ...
                    if(m_obs->ValidMove(GetMove())
                       && m_obs->GetTactics(GetMove()).GetProperty(m_square_class)
                       && StaticTactics::FullBlocker(*m_obs, GetMove(), TProperty::D4)){
                        m_valid = true;
                        m_empty = false;
                        return;
    ...
    public:
        MoveGenerator(const SquareObserver& obs, Piece own, TProperty square_class){
            assert(!StaticTactics::InstantLoss(m_obs, m_own));
            if(Move mv; StaticTactics::SingleMove(mv, obs, own)){
                ...
                new(&helpers.full) UnlimitedHelper(obs, own, square_class);
                active_helper = &helpers.full;
            }
            else{
                new(&helpers.d4) D4BlockerHelper(obs, own, square_class);
